...
class BrandsService:
    ...
    async def create(self, data: Dict[str, Any], user_id: Optional[str] = None) -> Optional[Brands]:
        """Create a new brands"""
        try:
            if user_id:
    ...
    async def check_ownership(self, obj_id: int, user_id: str) -> bool:
        ...
        try:
            ...
            logger.error(f"Error checking ownership for brands {obj_id}: {str(e)}")
            return False

    async def get_by_id(self, obj_id: int, user_id: Optional[str] = None) -> Optional[Brands]:
        """Get brands by ID (user can only see their own records)"""
        try:
            query = select(Brands).where(Brands.id == obj_id)
            if user_id:
                query = query.where(Brands.user_id == user_id)
    ...
    async def get_list(
        ...
        """Get paginated list of brandss (user can only see their own records)"""
        try:
            query = select(Brands)
            ...
            if user_id:
                query = query.where(Brands.user_id == user_id)
                count_query = count_query.where(Brands.user_id == user_id)
    ...
    async def update(self, obj_id: int, update_data: Dict[str, Any], user_id: Optional[str] = None) -> Optional[Brands]:
        """Update brands (requires ownership)"""
        try:
            obj = await self.get_by_id(obj_id, user_id=user_id)
            if not obj:
    ...
    async def delete(self, obj_id: int, user_id: Optional[str] = None) -> bool:
        """Delete brands (requires ownership)"""
        try:
            obj = await self.get_by_id(obj_id, user_id=user_id)
            if not obj:
